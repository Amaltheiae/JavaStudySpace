import Online_school.FirstYearStudy;
import Online_school.SecondYearStudy;
import Online_school.ThirdYearStudy;

public class Main {


    public static void main(String[] args) {

        FirstYearStudy firstYearStudy = new FirstYearStudy();

        System.out.println("First Year Study:");
        System.out.println("Name of lecture - " + firstYearStudy.lecture);
//        System.out.println("Number of Teachers " + firstYearStudy.teacher);
//        System.out.println("Number of students " + firstYearStudy.student);
//        System.out.println("Number of homework " + firstYearStudy.student);


        SecondYearStudy secondYearStudy = new SecondYearStudy();

        System.out.println("Second Year Study:");
        System.out.println("Name of lecture - " + secondYearStudy.lecture);
//        System.out.println("Number of Teachers " + secondYearStudy.teacher);
//        System.out.println("Number of students " + secondYearStudy.student);
//        System.out.println("Number of homework " + secondYearStudy.student);


        ThirdYearStudy thirdYearStudy = new ThirdYearStudy();

        System.out.println("Third Year Study:");
        System.out.println("Name of lecture - " + thirdYearStudy.lecture);
//        System.out.println("Number of Teachers " + thirdYearStudy.teacher);
//        System.out.println("Number of students " + thirdYearStudy.student);
//        System.out.println("Number of homework " + thirdYearStudy.student);


    }
}